...
``torchrec`` is pure Python3 code, it does not contain any C modules.
...
The default usage is via the `~torchrec.render_network` wrapper function.
...
The render_network function calls `~torchrec.record` and `~torchrec.make_dot` so the call to 
`~torchrec.render_network`  in the above example could be written as below, to allow for 
...
as keyword arguments to `~torchrec.render_network` (or `~torchrec.make_dot`\ ). 
...
If the default styling of node shapes/colors is not sufficient, you can create a subclass of `~torchrec.renderer.GraphvizStyler` and
pass it to `~torchrec.make_dot` via the ``styler_cls`` argument. The subclass needs to accepts `graphviz` attributes as keyword arguments,
and override the `~torchrec.renderer.GraphvizStyler.style_node` and `~torchrec.renderer.GraphvizStyler.style_edge` methods.
...
Currently, the `torchrec` package only provides rendering into `graphviz` objects, but the rendering functionality 
can be extended by subclassing the `~torchrec.renderer.base.BaseRenderer` class in a manner similar to the `~torchrec.renderer.GraphvizRenderer`.
You can read the source code to see how the subclassing can be done.

If you create a subclass of `~torchrec.renderer.base.BaseRenderer` for a new rendering format, submit a pull request! 
